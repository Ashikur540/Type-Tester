...
document.getElementById('placeHolder').style.display='none'
   // play sound
  if (newLetter && toplay) document.getElementById("sound").play();
...
const gameOver = () => {
  ...
  // words counter
  const wpm = wordcounter(timeTaken);
  // console.log(wpm);

  // console.log(typeController());
  // show result modal
  ...
  resultModal.innerHTML += `
    ...
    <p>WPM: <span class="bold red">${wpm}</span></p>
...
// wordcounter
const wordcounter = (time) => {
  // const timeSpent = Math.round((currentTime - startTime) / 1000);
  const totalWords = display.innerText;
  console.log(totalWords);

  let response = totalWords.split("▪").length;
  console.log(response);

  let wpm = Math.round((response / time) * 60)
  // console.log(wpm);
  return wpm;
}
...
function change() {
  // if true let them false or if false then true...
  toplay = !toplay
